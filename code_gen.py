...
class SourceGenerator(ExplicitNodeVisitor):
    ...
    def __init__(self, indent_with, add_line_information=False,
        pretty_string=pretty_string, len=len, isinstance=isinstance,
        callable=callable):
        self.result = []
            ...
            for item in params:
                if isinstance(item, AST):
                    ...
                    if item:
                        append(item)
        self.write = write
    def __getattr__(self, name, defaults=dict(keywords=(), _pp=Precedence.
        highest).get):
    ...
    def visit_ImportFrom(self, node):
        self.statement(node, 'from ', node.level * '.', node.module or '',
            ' import ')
        self.comma_list(node.names)
        if node.module == '__future__' and 'unicode_literals' in (x.name for
            x in node.names):
    ...
    def visit_ClassDef(self, node):
        ...
        for keyword in self.get_keywords(node):
            self.write(paren_or_comma, keyword.arg or '', '=' if keyword.
                arg else '**', keyword.value)
    ...
    def visit_For(self, node, is_async=False):
        ...
        prefix = 'async ' if is_async else ''
        self.statement(node, '%sfor ' % prefix, node.target, ' in ', node.
            iter, ':')
    ...
    def _handle_string_constant(self, node, value, is_joined=False):
        ...
        embedded = (precedence > Precedence.Expr) + (precedence >=
            Precedence.Assign)
        self.write('')
        result = self.result
    ...
    def _handle_numeric_constant(self, value):
        ...
        if isinstance(x, complex):
            ...
            s = real
        self.write(s)
    def visit_Num(self, node, new=sys.version_info >= (3, 0)):
    ...
    def visit_Dict(self, node):
        ...
        with self.delimit('{}'):
            for idx, (key, value) in enumerate(zip(node.keys, node.values)):
                self.write(', ' if idx else '', key if key else '', ': ' if
                    key else '**', value)
