import sys, os, ast, shutil, logging
from .code_gen import to_source
from file_util import code_to_ast
from .node_util import allow_ast_comparison, dump_tree, strip_tree, fast_compare
dsttree = 'tmp_rtrip'
...
def convert(srctree, dsttree=dsttree, readonly=False, dumpall=False,
    ...
    for srcpath, fname in allfiles:
        ...
        if dumpall or bad:
            ...
            logging.warning('    calculating dump -- %s' % ('bad' if bad else
                'OK'))
            if bad:
            ...
            if dumpall or bad:
                if not readonly:
                    ...
                    sys.stdout.write('\n\nNew AST:\n\n    ')
                    sys.stdout.write('(same as old)' if dstdump == srcdump else
                        dstdump.replace('\n', '\n    '))
                    sys.stdout.write('\n')
    if badfiles:
